...
import type { BlacklistUser, RestoreBlacklistOptions, RestoreResult } from '../types/api';
import {
  backupBlacklist,
  restoreBlacklist,
  clearBlacklist,
...
export default function Blacklist() {
  ...
  const [data, setData] = useState<BlacklistUser[]>([]);
  const [filteredData, setFilteredData] = useState<BlacklistUser[]>([]);
  ...
  const handleBackup = async () => {
    setLoading(true);
    try {
      const result = await backupBlacklist();
      setData(result);
      setFilteredData(result);
      message.success(`备份成功！共 ${result.length} 个黑名单用户`);
    } catch (error) {
  ...
  const handleRestore = async () => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        setLoading(true);
        try {
          const options: RestoreBlacklistOptions = {
            continue_on_error: true,
            batch_size: 10,
            delay_ms: 1000,
          };
          const result = await restoreBlacklist(data, options);
  ...
  const handleImport = async () => {
    try {
      ...
      if (filePath) {
        ...
        const imported = JSON.parse(content) as BlacklistUser[];
        setData(imported);
    ...
    <div>
      ...
      >
        <Row gutter={16} style={{ marginBottom: 16 }}>
          <Col span={12}>
            <Card>
              <Statistic title="黑名单总数" value={data.length} />
            </Card>
          </Col>
          <Col span={12}>
            <Card>
